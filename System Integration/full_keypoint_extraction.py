...
def extract_keypoints(video_path, show_video=False, target_length=1200, window_size=100):
    """
    Performs keypoint analysis from the video file and returns the results.

    Args:
        video_path (str): Path to the video file to be analyzed
        show_video (bool): Whether to display the video (default: False)
        target_length (int): Target length for sliding window (default: 1200)
    ...
    if not cap.isOpened():
        raise ValueError(f"Video file could not be opened: {video_path}")
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        while cap.isOpened():
            ...
            if results.pose_landmarks:
                ...
                angle_3d = calculate_angle_3d(x1, y1, z1, x2, y2, z2)
                full_analysis["hip_angle_list_3d"].append(angle_3d)
                ...
                heel_angle = calculate_angle(x1, y1, x2, y2)
                full_analysis["heel_angle_list_2d"].append(heel_angle)
                # ----------------------------------------------------------------------------------------------
                ...
                heel_angle_3d = calculate_angle_3d(x1, y1, z1, x2, y2, z2)
                full_analysis["heel_angle_list_3d"].append(heel_angle_3d)
                # ----------------------------------------------------------------------------------------------
                ...
                full_analysis["lfa_2d"].append(left_foot_angle)
                full_analysis["rfa_2d"].append(right_foot_angle)
                # ----------------------------------------------------------------------------------------------
                ...
                left_toe_coords = (
                    left_foot_index.x * frame.shape[1], left_foot_index.y * frame.shape[0],
                    left_foot_index.z * frame.shape[1])
                right_toe_coords = (right_foot_index.x * frame.shape[1], right_foot_index.y * frame.shape[0],
                                    right_foot_index.z * frame.shape[1])

                left_angle_3d = calculate_angle_3d(left_heel_coords[0], left_heel_coords[1], left_heel_coords[2],
                                                   left_toe_coords[0], left_toe_coords[1], left_toe_coords[2])
                right_angle_3d = calculate_angle_3d(right_heel_coords[0], right_heel_coords[1], right_heel_coords[2],
                                                    right_toe_coords[0], right_toe_coords[1], right_toe_coords[2])

                full_analysis["lfa_3d"].append(left_angle_3d)
                full_analysis["rfa_3d"].append(right_angle_3d)
                # ----------------------------------------------------------------------------------------------
                ...
                full_analysis["left_knee_angle_list"].append(left_knee_angle)
                full_analysis["right_knee_angle_list"].append(right_knee_angle)
                ...
                full_analysis["left_foot_lift_heights"].append(left_height)
                full_analysis["right_foot_lift_heights"].append(right_height)
    ...
    for key in full_analysis.keys():
        full_analysis[key] = extend_with_sliding_window(full_analysis[key], target_length, window_size)
...
if __name__ == "__main__":
    ...
    try:
        results = extract_keypoints(video_path, show_video=True)
