...
model = load_model("best_trained_tandem_walk_model.h5")

with open("tandem_walk_scaler.pkl", "rb") as f:
...
# -------------------- Inference Function --------------------
def run_inference_from_analysis_for_tandem(full_analysis, patient_id="Unknown"):
    try:
        matrix = doc_to_matrix(full_analysis)
        matrix_scaled = scaler.transform(matrix)
        matrix_scaled = np.expand_dims(matrix_scaled, axis=0)  # Add batch dimension
